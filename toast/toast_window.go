...
func (n *Notification) Push() error {
	if strings.HasSuffix(strings.ToLower(n.AppID), ".exe") {
		n.AppID = n.AppID[:len(n.AppID)-4]
	}
	n.applyDefaults()
	xml, err := n.buildXML()
	if err != nil {
...
func invokeTemporaryScript(content string) (err error) {
	dir := os.TempDir()
	var f *os.File
	if f, err = os.CreateTemp(dir, "*.ps1"); err != nil {
		return
	}
	file := f.Name()
	...
	c := exec.Command("PowerShell", "-ExecutionPolicy", "Bypass", "-File", file)
